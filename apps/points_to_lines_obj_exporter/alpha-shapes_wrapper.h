...
std::vector<Segment> generate_alpha_shape(std::vector<cgal_point_2> const& points){
	Alpha_shape_2 A(points.begin(), points.end(), FT(0.2), Alpha_shape_2::GENERAL);
...
std::vector<line> do_output_conversion_to_line_segments(std::vector<Segment> const& segments, float prjected_coord){
	...
	for(uint i = 0; i < segments.size() - 1; ++i){
		...
		auto current_cgal_start_point = current_segment.point(0);
		auto current_cgal_end_point = current_segment.point(1);

		float start_point_coordinstes[3] = {float(current_cgal_start_point.hx()), prjected_coord, float(current_cgal_start_point.hy())};
		point start_point(start_point_coordinstes);
...
std::vector<point> do_output_conversion_to_sorted_points(std::vector<Segment> const& segments, float prjected_coord){
